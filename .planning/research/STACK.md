...
**Confidence:** HIGH (core stack), MEDIUM (AI inference, payments), HIGH (email)
...
**Recommendation: Toss Payments directly (not via PortOne aggregator)**
...
**Why NOT PortOne:** PortOne is a payment orchestrator (aggregates Toss + KG Inicis + others). Useful if you need to support 10+ Korean PGs simultaneously. For ArtBridge v1 targeting Korean domestic users with Toss as primary PG, the added abstraction layer and additional pricing (PortOne shifted to usage-based pricing Jan 2025) adds cost and complexity with no benefit. Add PortOne later if you need multi-PG routing.

**Why NOT Stripe:** Stripe does not support Korean cards (BC, Shinhan, KB) or bank transfer (계좌이체) natively. Korean consumers expect Korean payment UX. Stripe is appropriate only if you plan to expand internationally before v1.

### AI Inference (ControlNet / Inpainting)

**Recommendation: fal.ai for managed API + RunPod Serverless for custom ComfyUI workflows**
...
**Why NOT Replicate:** Replicate is the most expensive option at scale (10K images/day pricing is highest of three). Cold starts can be unpredictable. No advantage over fal.ai for managed use cases.

**Why NOT self-host (on your own GPU server):** Unwarranted ops burden for MVP. Revisit after 10K+ daily inference calls.
...
**Why NOT Cloudinary:** Cloudinary is the superior image CDN/DAM, but costs escalate quickly (credit system combining storage + bandwidth + transformations). Supabase Storage's imgproxy transformations cover ArtBridge's needs (resize thumbnails, serve WebP). Add Cloudinary only if advanced AI-powered tagging/auto-cropping is needed post-MVP.

**Why NOT a separate Cloudflare Images account:** Adds billing complexity when Supabase Storage CDN is already included. Revisit if storage costs become significant at scale.
...
**Why NOT Vercel Cron:** Vercel Cron is available on Pro plan and works, but it introduces a second billing relationship and requires the job logic to live in Next.js API routes rather than closer to the database. pg_cron fires inside Postgres — the winner determination query and status update are atomic.

**Why NOT external job queues (BullMQ, Inngest):** Unnecessary for ArtBridge's scale. One auction closes at a time; there is no fan-out problem. Add a queue system if you later need parallel image processing at scale.
...
**Why NOT SendGrid:** SendGrid removed its free tier in May 2025. For an MVP with low email volume, paying $19.95/month for email before the product has users is a bad tradeoff. SendGrid's marketing email features are irrelevant for ArtBridge v1 (transactional only). Reconsider SendGrid if you need dedicated IPs for deliverability at very high volume.
...
**Why NOT Redux Toolkit:** Redux is appropriate for very large teams and complex state graphs. ArtBridge has a simple state model (user + one auction at a time). Redux overhead is unjustified.

**Why NOT TanStack Query:** TanStack Query (React Query) excels at client-side data fetching with caching. In Next.js App Router, RSC handles most data fetching server-side. TanStack Query is useful if you have heavy client-side polling. Supabase Realtime subscription handles live bid updates — adding TanStack Query for the same data would be redundant.
...
**Why NOT Datadog:** Datadog requires APM at $31/host/month minimum. Unjustified for MVP. Revisit when you have dedicated infrastructure and a team of 5+.

**Why NOT OpenTelemetry self-hosted stack:** Full OTel pipeline (collector + backend like SigNoz) has meaningful ops overhead. Supabase is adding OTel support to their products — follow their roadmap. Migrate to OTel-based stack when the team is ready to manage it.
...
```bash
# Bootstrap project
npx create-next-app@latest artbridge --typescript --tailwind --app --turbopack
...
# UI + Utilities
npm install lucide-react date-fns sharp
...
**Scenario A — Artist uploads artwork for promotional mockup:**
...
**Scenario B — Consumer space composition with accurate scale:**
...
**Scenario C — Real-time auction bidding:**
...
**Scenario D — Payment flow (fixed price or post-auction):**
...
*Stack research for: ArtBridge — AI-powered art e-commerce + real-time auction platform (Korean market)*
*Researched: 2026-05-10*
